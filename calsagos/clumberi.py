...
def clumberi(id_galaxy, ra_galaxy, dec_galaxy, redshift_galaxy, cluster_initial_redshift, ra_cluster, dec_cluster, range_cuts):
    ...
    for cv_type in cv_types:
        ...
        for n_components in n_components_range:
            ...
            gmm = mixture.GaussianMixture(n_components=n_components, covariance_type=cv_type, tol=1e-2, reg_covar=1e-10, max_iter=1000, n_init=10, random_state=6)
   ...
   # -- estimating the combined distance of redshift and position
    ...
    while True:
        # -- estimating the velocity dispersion for cleaned cluster sample
        sigma_clean_sample = biweight_scale(redshift_member)
        
        #--computing the cluster redshift
        new_cluster_redshift = biweight_location(redshift_member)

        # -- removing all gaaxies at more than 3 x sigma from the cluster redshift
        cluster_sample = np.where( (redshift_member > new_cluster_redshift-3*sigma_clean_sample) & (redshift_member < new_cluster_redshift+3*sigma_clean_sample) )[0]
        outliers = np.where( (redshift_member <= new_cluster_redshift-3*sigma_clean_sample) | (redshift_member >= new_cluster_redshift+3*sigma_clean_sample) )[0]

        # -- re-defining sample of redshifts and computing cluster redshift
        redshift_member = redshift_member[cluster_sample]
    ...
    # -- re-defining sample of cluster members
    id_member = id_galaxy[final_cluster_sample]
    ra_member = ra_galaxy[final_cluster_sample]
    dec_member = dec_galaxy[final_cluster_sample]
    redshift_member = redshift_galaxy[final_cluster_sample]
    
    # -- print the size of the cluster members
    print("cluster members:", len(redshift_member))

    # -- print the uncertainty on member selection
...
def clumberi_position(id_galaxy, ra_galaxy, dec_galaxy, ra_cluster, dec_cluster, range_cuts):
    
    """ CLUMBERI_position is a modified version of CLUMBERI
    (CLUster MemBER Identifier) which is a function that 
    selects members in a single cluster using Gaussian 
    Mixture Modules (GMM) in 3-dimension. However in this 
    version we have used only position of galaxies to select
    cluster members.

    This function was developed by D. Olave-Rojas (01/06/2025)

    CLUMBERI_position uses as input the position of the
    galaxies and the central position of the clsuter

    clumberi_position(id_galaxy, ra_galaxy, dec_galaxy, ra_cluster, 
    dec_cluster, range_cuts)
	...
	:returns: array with the cluster members and the probability
        of each galaxy to belong to the cluster
	:rtype: array

    .. note::

    The output of this module is:
    clumberi(id_galaxy, ra_galaxy, dec_galaxy, ra_cluster, 
        dec_cluster, 
        range_cuts)[0]: corresponds to the ID of the cluster 
        members

    clumberi(id_galaxy, ra_galaxy, dec_galaxy, ra_cluster, 
        dec_cluster, range_cuts)[1]: corresponds to the R.A. 
        of the cluster members
    
    clumberi(id_galaxy, ra_galaxy, dec_galaxy, ra_cluster, 
        dec_cluster, range_cuts)[2]: corresponds to the Dec 
        of the cluster members

    clumberi(id_galaxy, ra_galaxy, dec_galaxy, ra_cluster, 
        dec_cluster, range_cuts)[3]: corresponds to the probability
        of each galaxy to belong to the cluster member

	"""

    print("..starting CLUMBERI_position..")
    
    # -- creating a transposed array with the redshift_galaxy to be used as input in the GMM implementation
    candidate_galaxies = np.array([ra_galaxy, dec_galaxy]).T #-- This is a 3D-array, which considers the position and redshifts of the galaxies
    ...
    # -- selecting the closest Gaussian in position to the center of the cluster
    dim = angular_distance.size
    
    for ii in range(dim):
        if (angular_distance[ii] == min_angular_distance):
    ...
    #-- selecting the position of the galaxies within the closest Gaussian to the center of the cluster
    ra_initial = ra_galaxy[final_sample]
    dec_initial = dec_galaxy[final_sample]

    #-- estimating the dispersion of the redshift distribution of the closest Gaussian to the center of the cluster
    sigma_ra = biweight_scale(ra_initial)
    sigma_dec = biweight_scale(dec_initial)

    #-- estimating the central redshift of the redshift distribution of the closest Gaussian to the center of the cluster
    ra = biweight_location(ra_initial)
    dec = biweight_location(dec_initial)

    # -- removing all galaxies at more than 3 x sigma from the cluster redshift distribution of galaxies 
    cluster_sample = np.where( (ra_galaxy > (ra -3*sigma_ra)) & (ra_galaxy < + (ra + 3*sigma_ra)) & (dec_galaxy > (dec -3*sigma_dec)) & (dec_galaxy < + (dec + 3*sigma_dec)) )[0]
    ...
    # --computing the cluster redshift
    new_cluster_ra = biweight_location(ra_member)
    new_cluster_dec = biweight_location(dec_member)
 
    while True:
        # -- estimating the velocity dispersion for cleaned cluster sample
        sigma_clean_ra_sample = biweight_scale(ra_member)
        sigma_clean_dec_sample = biweight_scale(dec_member)
        ...
        # -- removing all gaaxies at more than 3 x sigma from the cluster redshift
        cluster_sample = np.where( (ra_member > new_cluster_ra-3*sigma_clean_ra_sample) & (ra_member < new_cluster_ra+3*sigma_clean_ra_sample) & (dec_member > new_cluster_dec-3*sigma_clean_dec_sample) & (dec_member < new_cluster_dec+3*sigma_clean_dec_sample) )[0]
        outliers = np.where( (ra_member <= new_cluster_ra-3*sigma_clean_ra_sample) | (ra_member >= new_cluster_ra+3*sigma_clean_ra_sample) | (dec_member <= new_cluster_dec-3*sigma_clean_dec_sample) | (dec_member >= new_cluster_dec+3*sigma_clean_dec_sample))[0]

        # -- re-defining sample of redshifts and computing cluster redshift
        ra_member = ra_member[cluster_sample]
        dec_member = dec_member[cluster_sample]
    ...
    # -- removing all galaxies at more than 3 x sigma from the cluster redshift distribution of galaxies 
    final_cluster_sample = np.where( (ra_galaxy > (new_cluster_ra -3*sigma_clean_ra_sample)) & (ra_galaxy < + (new_cluster_ra + 3*sigma_clean_ra_sample)) & (dec_galaxy > (new_cluster_dec -3*sigma_clean_dec_sample)) & (dec_galaxy < + (new_cluster_dec + 3*sigma_clean_dec_sample)) )[0]
    # -- re-defining sample of cluster members
    id_member = id_galaxy[final_cluster_sample]
    ra_member = ra_galaxy[final_cluster_sample]
    dec_member = dec_galaxy[final_cluster_sample]
    ...
    # -- print the size of the cluster members
    print("cluster members:", len(ra_member))

    # -- print the uncertainty on member selection
    print("sigma members:", round(len(ra_member)/100.))

    #-- print the central redshift of the cluster estimated using galaxy members selected with GMM
    print("new_cluster_RA:", new_cluster_ra)
    print("new_cluster_Dec:", new_cluster_dec)

    # -- building matrix with output quantities
    cluster_array = np.array([id_member, ra_member, dec_member], dtype=object)
