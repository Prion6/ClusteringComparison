...
def calc_knn_great_circle_distance(RA, DEC, knn):

    """ calsagos.utils.calc_knn_great_circle_distance(RA, DEC, knn)

	Function that computes the angular
    distance between one galaxy and their 
    k-nearest neighbors using the great circle
    distance as the distance between galaxies
	...
	:returns: the distance in degrees between 
        one galaxy and its K-Nearest Neighbors 
        and the mean distance between one galaxy 
    ...
    The angular galaxy distance is estimated as 
    the great circle distance between one galaxy 
    and all galaxies in the sample.
     
    The separation is calculated using the Vicenty
    equation implemented in SkyCoord from astropy

    The output of this module is:

    calc_knn_great_circle_distance(RA, DEC, knn)[0] corresponds to the
        distance from a galaxy to its K-Nearest Neighbor

    calc_knn_great_circle_distance(RA, DEC, knn)[1] corresponds to the
        mean distance between a galaxy and its K-Nearest Neighbors
    ...
    # -- defining galaxy coordinates with SkyCoord
...
#            print((coordinates[ii]))            
#            # -- estimating the distance between galaxies in the sample
#            distance[jj] = coordinates[ii].separation(coordinates[jj])
...
# -- NEW FUNCTION NO TESTED YET -- #
#####################################################################################################################################################################################
#####################################################################################################################################################################################

def calc_knn_redshift_galaxy_distance(redshift, knn):

    """ calsagos.utils.calc_knn_redshift_galaxy_distance(redshift, knn)

	Function that computes the redshift
    distance between one galaxy and their 
    k-nearest neighbors
	...
	:param redshift: redshift of the galaxies
    :param knn: k nearest neighbors

    :type redshift  : array
    :type knn       : int

	:returns: the redshift distance between 
        one galaxy and its K-Nearest Neighbors
        and the median redshift distance between 
        one galaxy and their k-nearest neighbors
    :rtype: numpy array

    .. note::

    The redshift disstance is estimated as the
    absolute difference between one galaxy and 
    the all galaxies in the sample, then this 
    function selects the K-Nearest Neighbors to a 
    each galaxies.
    ...
    calc_knn_redshift_galaxy_distance(redshift, knn)[0] corresponds to the
        redshift distance from a galaxy to its K-Nearest Neighbor

    calc_knn_redshift_galaxy_distance(redshift, knn)[1] corresponds to the
        mean redshift distance between a galaxy and its K-Nearest Neighbors

	:Example:
        >>> import numpy as np
        >>> redshift = np.array([0.342669882944, 0.337408456897, 0.33848153958, 
        0.343682270793, 0.334047304888, 0.340701046186, 0.335555781738, 
        0.336091267789, 0.334590641617, 0.343946501271])
        >>> from calsagos import utils
        >>> distance = utils.calc_knn_redshift_galaxy_distance(redshift, 3)
        >>> print("knn_distance: ", distance[0])
        knn_redshift_distance:  [0.0012766183269999898 0.0013171891079999942 
        0.00221950660600001 0.001012387848999985 0.0015084768499999957 
        0.00221950660600001 0.0009651401210000188 0.0013171891079999942 
        0.0009651401210000188 0.0012766183269999898]
        >>> print("mean_knn_distance: ", distance[1])
        median_knn_redshift_distance:  [0.001012387848999985 0.0010730826829999929 
        0.0010730826829999929 0.0002642304780000049 0.0005433367289999769 
        0.0019688367580000254 0.0005354860510000115 0.0005354860510000115 
        0.0005433367289999769 0.0002642304780000049]

	"""

    # -- defining output quantities
    dim = redshift.size # -- number of elements in the input arrays
    distance = np.zeros(dim) # -- distance between K-Nearest Neighbors
    median_distance = np.zeros(dim) # -- mean distance between the K-Nearest Neighbors
    d_knn = np.zeros(dim)
    ...
    for ii in range(dim):
        ...
        for jj in range(dim):
            ...
            # -- estimating the distance between galaxies in the sample
            distance[jj] = np.sqrt((redshift[ii] - redshift[jj])**2.)
            ...
            # -- estimating the mean distance between the K-Nearest Neighbors
            median_distance[ii] = np.median(d_nearest_k)
    ...
    # -- defining the mean distance between a galaxy from a its K-Nearest Neighbors
    angular_distance = median_distance
...
def best_eps_redshift_dbscan(id, angular_distance, redshift_distance):

    """ calsagos.utils.best_eps_dbscan(id_galaxy, angular_distance_galaxy, redshift_distance_galaxy)
	...
	:param id: ID of each galaxy in the sample
    :param angular_distance: angular distance 
        between galaxies
    :param redshift_distance: redshift distance 
        between galaxies

    :type id_galaxy: array
    :type angular_distance: array
    :type redshift_distance: array
    ...
    # -- sorting the distances from a galaxy to it k-nearest neighbor
    sorted_indices = np.argsort(angular_distance)
#    galaxy_angular_distance = np.sort(angular_distance, axis=0)
    galaxy_angular_distance = angular_distance[sorted_indices]
    galaxy_redshift_distance = redshift_distance[sorted_indices]

#    print(galaxy_angular_distance**2.)
#    print(type(galaxy_angular_distance))
#    print(galaxy_redshift_distance)
    ...
    # -- creating an array with index and the distances to the k-nearest neighbor
    array_id_distance = np.array([number, galaxy_redshift_distance]).T
    ...
    # -- selecting the elbow distance in the distribution
    best = np.where(number == elbow_idx)[0]
    best_eps = galaxy_redshift_distance[best]
...
# FUNCTION THAT PERFORMS A NUMBER OF MONTE-CARLO SIMULATIONS ON EACH ENTRY OF AN ARRAY
# THIS FUNCTION IS BASED ON run_Monte_Carlo_simulation WRITTEN BY Pierluigi Cerulo (12/05/2016)
# SOME MODIFICATIONS WERE MADE BY D. Olave-Rojas (02/04/2018)
...
# FUNCTION THAT CREATES ARRAYS WITH REPEATED ENTRIES
# (Pierluigi Cerulo 14/05/2016)
