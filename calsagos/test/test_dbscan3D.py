...
def test_calsagos_clumberi(id_galaxy, ra_galaxy, dec_galaxy, redshift_galaxy, cluster_mass, cluster_initial_redshift, ra_cluster, dec_cluster, input_H, input_Omega_L, input_Omega_m, range_cuts, n_galaxies, flag, final_cluster_catalog):
    ...
    knn_redshift = utils.calc_knn_redshift_galaxy_distance(redshift_member, n_galaxies)

    #-- determining the distance to the k-nearest neighbor of each galaxy in the cluster
    knn_galaxy_distance = knn_distance[0]
    knn_galaxy_redshift = knn_redshift[0]

    typical_distance_separation = utils.best_eps_dbscan(id_member, knn_galaxy_distance)
    typical_redshift_separation = utils.best_eps_redshift_dbscan(id_member, knn_galaxy_distance, knn_galaxy_redshift)
    typical_separation = np.sqrt(typical_distance_separation**2. + typical_redshift_separation**2.)

    print(typical_distance_separation, typical_redshift_separation)
    print(typical_separation)

    #-- Assign galaxies to each substructures
    label_candidates = lagasu.lagasu_dbscan_3D(id_member, ra_member, dec_member, redshift_member, typical_separation, n_galaxies, ra_cluster, dec_cluster, cluster_redshift, r200_degree, flag)
    ...
    label_dbscan = label_candidates[4] #-- label of each galaxy in the catalog with cluster members. This parameter is given by DBSCAN
    label_final = label_candidates[5] #-- label of each galaxy in the catalog with cluster members. Equal to -1 if the galaxy is part of the principal halo and has a value between 0 to N if th galaxy is part of a substructure 
...
#-- OUTPUT CATALOGS
final_output_catalog = '/home/dolaver/python_programs/mymodules/calsagos/test/output_CALSAGOS_CLUMBERI_LAGASU3D_catalogue.cat'
