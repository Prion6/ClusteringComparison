...
def test_calsagos_clumberi(id_galaxy, ra_galaxy, dec_galaxy, redshift_galaxy, cluster_mass, cluster_initial_redshift, ra_cluster, dec_cluster, input_H, input_Omega_L, input_Omega_m, range_cuts, n_galaxies, metric_distance, flag_distance, flag, final_cluster_catalog):
    ...
    # -- defining the used metric and input parameters
    if metric_distance == 'euclidean':
        cluster_sample = np.array([ra_member, dec_member]).T
    if metric_distance == 'haversine':
        ra = np.radians(ra_member)
        dec = np.radians(dec_member)
        cluster_sample = np.array([ra, dec]).T

    # -- estimating the galaxy separation of galaxies in the cluster sample to be used as input in lagasu
    neigh = NearestNeighbors(n_neighbors=n_galaxies, metric=metric_distance).fit(cluster_sample)
    ...
    knn_galaxy_distance = distances[:,(n_galaxies-1)]

    if flag_distance == 'elbow ':
        typical_separation = utils.best_eps_dbscan(id_member, knn_galaxy_distance)

    if flag_distance == 'median':
        typical_separation = np.median(knn_galaxy_distance)

    if flag_distance == 'median_sigma':
        typical_separation = np.median(knn_galaxy_distance)+(1.*np.std(knn_galaxy_distance))
...
#    typical_separation = utils.best_eps_dbscan(id_member, knn_galaxy_distance)
    ...
    label_candidates = lagasu.lagasu(id_member, ra_member, dec_member, redshift_member, range_cuts, typical_separation, n_galaxies, metric_distance, ra_cluster, dec_cluster, cluster_redshift, r200_degree, flag)
    ...
    print("# ID RA DEC redshift dbscan groups")

    for ii in range(n_members):

        print(id_candidates[ii], ra_candidates[ii], dec_candidates[ii], redshift_candidates[ii], label_dbscan[ii], label_final[ii])
...
distance_type = 'elbow'
#-- OUTPUT CATALOGS
final_output_catalog = '/home/dolaver/python_programs/mymodules/calsagos/test/output_CALSAGOS_CLUMBERI_catalogue_euclidean_median.cat'
...
test_calsagos_clumberi(ID, RA, DEC, redshift, cluster_mass, starting_redshift, central_ra, central_dec, cluster_H_mock, cluster_Omega_L_mock, cluster_Omega_m_mock, cut, ngal, metric_cluster, distance_type, sample, final_output_catalog)
